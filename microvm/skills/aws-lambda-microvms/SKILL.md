...
owner_team: Lambda
owner_cti: AWS/Lambda MicroVMs/General
stages: [preprod]
version: 1
metadata:
  service: [lambda]
...
# Run a MicroVM (returns endpoint + microVmId)
...
TOKEN=$(aws lambda-micro-vms create-micro-vm-auth-token \
  --micro-vm-identifier mvm-... --expiration-in-minutes 30 \
...
See [`references/getting-started.md`](references/getting-started.md) for the full walkthrough including `--runtime` config and lifecycle hooks.

## Hook configuration

Hooks are organized into two groups under `runtime.hooks`:
...
> For all other quotas — concurrent MicroVMs per account, launch rate, image count, max execution duration, auth token TTL, LNC limits, per-ENI bandwidth, etc. — **check the AWS docs / Service Quotas console.** Most are soft quotas, raisable through Service Quotas / Support.

## Additional capabilities

By default, the container runs with a restricted set of Linux capabilities. Set `additionalCapabilities: ["ALL"]` under `--runtime` at image creation time only when required by your use case:
...
aws lambda-micro-vms create-micro-vm-image \
  ...
  --build-role-arn arn:aws:iam::<acct>:role/MicroVMBuildRole \
  --code-artifact '{"uri":"s3://<bucket>/<key>.zip"}' \
  --runtime '{"additionalCapabilities":["ALL"]}'
```
...
- **Image versions incur storage cost** even when no MicroVMs are running on them. Use `delete-micro-vm-image-version` to clean up.
...
- **`/run` hook is fast-notification only.** Don't use it for slow init — that work belongs at image build time.
...
- [`references/iam-and-security.md`](references/iam-and-security.md) — build role vs. execution role, trust policies, auth tokens (regular vs. shell), `iam:PassNetworkConnector`.
