...
A single image version can produce multiple `Build` records — one per `(architecture, chipset, chipsetGeneration)`. Each build has its own `buildState`: `PENDING` → `IN_PROGRESS` → `SUCCESSFUL` | `FAILED`. List with `list-micro-vm-image-builds`.
...
Your application can implement HTTP endpoints that Lambda invokes at lifecycle transitions. Hooks are **opt-in per image** in the `runtime.hooks` config. You must specify the `port` your hooks listen on (commonly `9000`).
...
> Always set explicit timeout values in `--runtime` when creating the image — especially for `/ready` (init) and `/run`/`/resume` (any per-VM init work).
...
- **Don't use `/run` for long-running init.** That work belongs at image build time so it's captured in the snapshot. Avoid generating random data at build time — if unique random state is needed, invalidate and re-generate using a CPRNG on `/run`.
