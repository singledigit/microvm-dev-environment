...
2. **Generate it in `/run`.** This hook fires once after run (post-snapshot resume) and is the canonical place to create per-VM unique state. Set `runHook: ENABLED` in `runtime.hooks`.
...
|---|---|---|---|---|
| **Build env vars** (`runtime.environmentVariables`) | Image creation | Yes — burnt into the snapshot | Container env at build time and after resume | Static, non-sensitive: log level, app port, feature toggles |
...
```bash
BUILD_ID=$(aws lambda-micro-vms list-micro-vm-image-builds \
  --image-identifier my-image \
  --image-version 1 \
  --query 'items[0].buildId' --output text)

aws lambda-micro-vms get-micro-vm-image-build \
  --image-identifier my-image \
  --image-version 1 \
  --build-id "$BUILD_ID"
```
